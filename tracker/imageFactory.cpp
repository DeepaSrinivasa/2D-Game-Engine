...
ImageFactory::~ImageFactory() {
  ...
  std::map<string,SDL_Surface*>::iterator si = surfaces.begin();
  while( si != surfaces.end() ){
    SDL_FreeSurface(si->second);
    ++si;
  }
  std::map<string,SDL_Texture*>::iterator ti = textures.begin();
  while( ti != textures.end() ){
    SDL_DestroyTexture(ti->second);
    ++ti;
  }
  std::map<string,Image*>::iterator fi = images.begin();
  while( fi != images.end() ){
